...
<template>
    <DialogBase width="500" v-model="data!" class="text-white">
        <template #title v-if="props.isEdit">
            ...
            {{ $t('modal.edit-Variable') }}
        </template>
        <template #title v-else>
            <v-icon>mdi-hammer</v-icon>
            {{ $t('modal.new-Variable') }}
        </template>
        <template #text>
            <v-text-field :error="props.titleError" v-model="props.targetData.name" :autofocus="true" required :label="$t('modal.enter-Variable-name')" hide-details></v-text-field>
